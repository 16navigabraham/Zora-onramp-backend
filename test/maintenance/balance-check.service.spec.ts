...
describe('BalanceCheckService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ConfigService, useValue: { get: () => ({ balanceCheck: { lowThresholdUsdc: 1, checkIntervalMinutes: 1, alertCooldownMinutes: 60, hysteresisUsdc: 0 } }) } },
      ],
    }).compile();
  ...
  it('should reset alerted state when balance recovers above hysteresis', async () => {
    ...
    await service['checkBalance']();
    // no new alert, but alerted state should be cleared
